...
common.setupOutgoing = function(outgoing, options, req, forward) {
  ...
  outgoing.path =
    typeof options[forward || 'target'].pathRewrite === 'function'
      ? options[forward || 'target'].pathRewrite(req.path)
...
function ws(req, socket, head, options) {
  common.setupSocket(socket);

  if (head && head.length) socket.unshift(head);

  // @ts-ignore
  const targetUrlObj = new URL(options.target);
  ...
  const opt = Object.assign(
    common.setupOutgoing(
      ...
      // @ts-ignore
      {
        target: Object.assign(targetUrlObj, options),
  ...
  const proxyReq = (common.isSSL.test(targetUrlObj.protocol)
    ? https
    : http
  ).request(opt);

  proxyReq.on('error', err => {
    console.log('>>> proxyReqOnError: ', err);
    socket.end();
  });
  ...
  proxyReq.on('upgrade', (res, proxySocket, proxyHead) => {
    proxySocket.on('error', err => {
      console.log('>>> proxySocketOnError: ', err);
      socket.end();
    });
    ...
    socket.write(
      createHttpHeader('HTTP/1.1 101 Switching Protocols', res.headers)
    );
